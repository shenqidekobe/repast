...
layui.use(['icheck', 'laypage','layer','reuse','form'], function() {
	...
	function loadList(page,pageSize){
		...
		$.ajax({
			url:ctx+'/admin/resource/listData',
			type:'POST',
	...
	function showForm(id){
		var loading = layer.load();
		$.get(ctx+'/admin/resource/edit', {id:id}, function(form) {
			layer.open({
				type: 1,
				title: '预约资源信息表单',
				content: form,
	...
	function remove(){
		var id=$(this).parent().attr("id");
		layer.confirm('确定要删除此预约资源嘛？', {icon: 3, title:'确认提示'}, function(index){
			var loading = layer.load();
			$.ajax({
				url:ctx+'/admin/resource/remove',
				type:'POST',
				data:{id:id},
				dataType:'json',
				success:function(data){
